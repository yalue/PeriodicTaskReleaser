...
void InitHistograms(int cellSizeX, int cellSizeY, int blockSizeX,
  ...
  float* weights = (float*) malloc(cellSizeX * blockSizeX * cellSizeY *
  ...
  cutilSafeCall(cudaMallocArray(&gaussArray, &channelDescGauss, cellSizeX *
    blockSizeX * cellSizeY * blockSizeY, 1));
  cutilSafeCall(cudaMemcpyToArray(gaussArray, 0, 0, weights, sizeof(float) *
    cellSizeX * blockSizeX * cellSizeY * blockSizeY, cudaMemcpyHostToDevice));
  free(weights);
  weights = NULL;
  int h_tvbin[3];
  ...
  // TODO: Can be 360
  h_cenBound[2] = 180 / 2.0f;
  h_halfBin[0] = blockSizeX / 2.0f;
  h_halfBin[1] = blockSizeY / 2.0f;
  h_halfBin[2] = noHistogramBins / 2.0f;
  h_bandWidth[0] = (float) cellSizeX;
  h_bandWidth[0] = 1.0f / h_bandWidth[0];
  h_bandWidth[1] = (float) cellSizeY;
  h_bandWidth[1] = 1.0f / h_bandWidth[1];
  // TODO: Can be 360
  h_bandWidth[2] = 180.0f / (float) noHistogramBins;
  h_bandWidth[2] = 1.0f / h_bandWidth[2];
  h_tvbin[0] = blockSizeX;
  h_tvbin[1] = blockSizeY;
  h_tvbin[2] = noHistogramBins;

  cutilSafeCall(cudaMemcpyToSymbol(cenBound, h_cenBound, 3 * sizeof(float), 0,
    cudaMemcpyHostToDevice));
  cutilSafeCall(cudaMemcpyToSymbol(halfBin, h_halfBin, 3 * sizeof(float), 0,
    cudaMemcpyHostToDevice));
  cutilSafeCall(cudaMemcpyToSymbol(bandWidth, h_bandWidth, 3 * sizeof(float),
    0, cudaMemcpyHostToDevice));
  cutilSafeCall(cudaMemcpyToSymbol(tvbin, h_tvbin, 3 * sizeof(int), 0,
    cudaMemcpyHostToDevice));
}
